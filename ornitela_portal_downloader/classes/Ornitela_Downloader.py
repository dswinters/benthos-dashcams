...
class Ornitela_Downloader:
    ...
    def __init__(self, config):
        ...
        self.output_dir = config['raw_output_directory']
    ...
    def download_month(self, serial, year, month, dtype=3,):
        from_time = datetime(year, month, 1, 0, 0, 0)
        to_time = datetime(year, month+1, 1, 0, 0, 0)

        request = self.gen_request(serial, from_time, to_time, dtype)
        response = self.s.post(self.url, data=request)
        if response.status_code == 200:
